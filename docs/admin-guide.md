...
| `sections` | 執筆者が記事を作成できるセクション。`dir` は `content/` 配下のフォルダ名、`label` は画面表示名。ここに載せていないフォルダも、記事があれば自動検出して一覧に表示されます (表示名はフォルダ名)。`label` を付けたいセクションだけ書けば十分です。 |
...
- セクション内は自由に階層化できます (例: `content/posts/2026/09/記事.md`)。
  一覧にはサブフォルダ名がバッジで表示されます。ページバンドル (`記事名/index.md`) にも対応しています。
- `content/` 直下の単独ページ (`about.md` など) は「その他のページ」として表示されます。
...
- リポジトリ直下に Hugo サイトがなくても大丈夫です。ユーザーがリポジトリのルートを選択すると、
  ...
  Git 操作 (取得・送信) は常にリポジトリのルートで行われます。
- 逆に、Hugo サイト本体のフォルダを直接選択していても、その親が Git リポジトリなら自動で認識します
  (`git rev-parse --show-toplevel` で検出)。状態チップにマウスを乗せると対象リポジトリのパスを確認できます。
