...
- **画像の添付 (ページバンドル方式)**: 記事に画像を挿入すると、通常記事 (`foo.md`) は自動的に
  ページバンドル (`foo/index.md`) へ変換され、画像ファイルは同じフォルダに保存されます。
  Markdown には `![](画像名.jpg)` という相対参照だけが書き込まれるため、記事ファイルは肥大化せず、
  Git の差分・履歴も健全に保たれます。ページの URL は変換前と変わりません。
  バンドル記事を削除すると画像もフォルダごとごみ箱に移動します。
- プレビューは下書きに加えて未来日付の記事も表示します (公開ビルドでは Hugo の標準どおり、
  未来日付の記事は公開日が来るまで除外されます)。
